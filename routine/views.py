...
from mainpage.views import bayesian_average
...
PROBLEM_TO_STEPS = {
    "acne": {
        'description': "برای پوست مستعد جوش ابتدا تونر استفاده می‌شود تا چربی پوست متعادل شده و منافذ پاکسازی شوند. بعد از آن سرم به‌کار می‌رود تا التهاب و جوش‌ها سریع‌تر کاهش یابند. در نهایت ضدآفتاب استفاده می‌شود تا از ایجاد لک و جای جوش جلوگیری کند. این ترتیب باعث می‌شود ابتدا بستر پوست آماده و متعادل شود، سپس درمان هدفمند انجام گیرد و در آخر محافظت در برابر آسیب‌های محیطی تکمیل گردد.",
        "تونر": {
    ...
    "oiliness": {
        'description': "در پوست‌های چرب ابتدا تونر استفاده می‌شود تا چربی اضافی کنترل و ظاهر مات روی پوست ایجاد شود. سپس سرم سبک آبرسان به‌کار می‌رود تا بدون ایجاد چربی اضافه، رطوبت لازم تأمین گردد. ترتیب مراحل به این صورت است که ابتدا کنترل چربی و آماده‌سازی پوست انجام می‌شود و سپس آبرسانی سبک برای تعادل بهتر رطوبت و چربی اضافه می‌شود.",
        "تونر": {
    ...
    "dryness": {
        'description': "برای خشکی پوست ابتدا سرم آبرسان استفاده می‌شود تا رطوبت عمیق وارد لایه‌های زیرین پوست شود. سپس کرم مرطوب‌کننده روی سرم زده می‌شود تا رطوبت را قفل کرده و از تبخیر آن جلوگیری کند. این ترتیب به این دلیل است که سرم به سرعت و عمق نفوذ می‌کند و کرم لایه محافظ ایجاد می‌کند.",
        "سرم": {
    ...
    "sensitivity": {
        'description': "در پوست‌های حساس، ابتدا سرم ترمیم‌کننده به کار می‌رود تا التهاب و ضعف سد پوستی کاهش یابد. سپس کرم مرطوب‌کننده ضدالتهاب روی آن زده می‌شود تا اثر آرامش‌بخش داشته و از تحریک بیشتر جلوگیری کند. ترتیب مراحل باعث می‌شود ابتدا درمان و تقویت پوست انجام شود و سپس با لایه محافظ کرم، اثر تثبیت گردد.",
        "سرم": {
    ...
    "hydration": {
        'description': "برای کمبود رطوبت پوست، ابتدا ماسک آبرسان شبانه استفاده می‌شود تا پوست عمقی و طولانی‌مدت هیدراته شود. سپس کرم مرطوب‌کننده سبک به کار می‌رود تا رطوبت در پوست حفظ گردد. ترتیب مراحل این‌گونه طراحی شده که ابتدا تأمین آب عمقی انجام گیرد و بعد از آن کرم به‌عنوان سد محافظ رطوبت عمل کند.",
        "ماسک": {
    ...
    "elasticity": {
        'description': "برای بهبود خاصیت ارتجاعی پوست، ابتدا سرم کلاژن‌ساز استفاده می‌شود تا تحریک ساخت کلاژن در لایه‌های زیرین انجام شود. سپس کرم لیفتینگ روی آن زده می‌شود تا اثر تقویتی و سفت‌کنندگی سطحی ایجاد گردد. در نهایت ضدآفتاب زده می‌شود تا از تخریب کلاژن توسط نور خورشید جلوگیری کند. این ترتیب باعث می‌شود درمان درونی آغاز شود، سپس حمایت سطحی انجام گیرد و در نهایت محافظت در برابر عوامل محیطی تکمیل گردد.",
        "سرم": {
    ...
    "redness": {
        'description': "برای کاهش قرمزی ابتدا سرم تسکین‌دهنده به کار می‌رود تا التهاب و تحریک را کاهش دهد. سپس کرم مرطوب‌کننده ضدالتهاب استفاده می‌شود تا سد پوستی تقویت شده و قرمزی پایدار کمتر شود. ترتیب مراحل باعث می‌شود ابتدا درمان عمقی التهاب انجام گیرد و سپس اثر محافظ و آرامش‌بخش کرم تثبیت گردد.",
        "سرم": {
...
def generate_full_plan(request):
    ...
    routine.append({
        ...
        'steps': [{
            ...
            # 'key_query': 'پاک',
            'description': 'یک پاک‌کننده مناسب پوست شما'
        }]
    })
    ...
    for order, skin_score in enumerate(skin_scores[:3]):
        ...
        sub_steps['description'] = PROBLEM_TO_STEPS[skin_score[0]]['description']
        sub_steps["steps"] = []
        for sub_step_name, sub_step_info in PROBLEM_TO_STEPS[skin_score[0]].items():
            if sub_step_name != 'description':
                sub_step = {}
...
HYDRATION_STEP_DESCRIPTIONS = {
    ...
    "hydrating_serum_light": {
        "why": "آبرسانی ملایم برای پوست‌های نرمال تا مختلط فراهم می‌کند بدون اینکه سنگین یا چسبناک باشد.",
        "consequence": "نبود آن ممکن است منجر به کاهش شادابی و دهیدراته شدن پوست شود."
    },
    "moisturizer": {
    ...
    "moisturizer_light": {
        "why": "رطوبت کافی را بدون ایجاد حس سنگینی یا براقیت روی پوست نرمال فراهم می‌کند.",
        "consequence": "نبود آن ممکن است منجر به دهیدراته شدن و کاهش لطافت پوست شود."
    }
}
...
def generate_hydration_plan(request):
    ...
    if hydration < -1 and dryness <= 1:
        routine.extend([
            {
                ...
                'description': "در این مرحله ابتدا سرم آبرسان استفاده می‌شود زیرا با نفوذ عمقی، رطوبت را به لایه‌های زیرین پوست می‌رساند و زمینه را برای ماندگاری بهتر رطوبت فراهم می‌کند.",
                'steps': [{
            ...
            },
            {
                ...
                'description': "پس از سرم، کرم مرطوب‌کننده به کار می‌رود تا لایه‌ای محافظ روی پوست ایجاد کند و رطوبت منتقل‌شده توسط سرم را قفل کند. این ترتیب باعث ماندگاری آبرسانی و جلوگیری از تبخیر آب پوست می‌شود.",
                'steps': [{
        ...
        routine.extend([
            {
                ...
                'description': "ابتدا از ژل سبک آبرسان استفاده می‌شود تا بدون ایجاد سنگینی، رطوبت پایه‌ای و ملایم به پوست برسد و از دهیدراته شدن جلوگیری شود.",
                'steps': [{
                    "step_name": "سرم آبرسان بسیار قوی",
            ...
            },
            {
                ...
                'description': "پس از آبرسانی سبک، تونر متعادل‌کننده به کار می‌رود تا PH پوست تنظیم شود و تعادل بین رطوبت و چربی حفظ گردد. این مرحله پوست را برای جذب بهتر مراحل بعدی آماده می‌کند.",
                'steps': [{
                    "step_name": "مرطوب‌کننده فوق غنی",
            ...
            },
            {
                ...
                'description': "در این مرحله سرم کنترل سبوم استفاده می‌شود تا ترشح چربی پوست کاهش یابد و از براق شدن یا ایجاد جوش جلوگیری کند. این مرحله در انتها قرار می‌گیرد تا روی پوست لایه‌ای سبک ولی موثر برای کنترل چربی بسازد.",
                'steps': [{
                    "step_name": "ماسک شب",
        ...
        routine.extend([
            {
                'order': 2,
                'step_name': 'سرم سبک آبرسان',
                'description': "برای پوست نرمال، استفاده از سرم سبک آبرسان کمک می‌کند تا رطوبت طبیعی پوست حفظ شود بدون اینکه احساس سنگینی یا چربی اضافی ایجاد کند.",
                'steps': [{
                    "step_name": "سرم سبک آبرسان",
                    "search_query": "سرم سبک آبرسان نرمال",
                    'description': HYDRATION_STEP_DESCRIPTIONS["hydrating_serum_light"]["why"] + ' ' + HYDRATION_STEP_DESCRIPTIONS["hydrating_serum_light"]["consequence"]
                }]
            },
            {
                'order': 3,
                'step_name': 'کرم مرطوب‌کننده ملایم',
                'description': "پس از سرم، یک مرطوب‌کننده سبک و متعادل استفاده می‌شود تا رطوبت را قفل کند و سد محافظ طبیعی پوست را تقویت نماید.",
                'steps': [{
                    "step_name": "مرطوب‌کننده سبک",
                    "search_query": "مرطوب کننده نرمال",
                    'description': HYDRATION_STEP_DESCRIPTIONS["moisturizer_light"]["why"] + ' ' + HYDRATION_STEP_DESCRIPTIONS["moisturizer_light"]["consequence"]
                }]
            }
        ])
...
def find_step_products(request, name):
    ...
    for step in routine_plan.steps:
        ...
        results.append([step['step_name'], subs, step['order'], step['description']])
